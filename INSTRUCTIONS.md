# ð¤ BOT-SYNC â Install Instructions (v0.7.10)
...
them. FTP / FTPS / SFTP work the same as the cloud backends â plain host +
credentials on the **âï¸ Accounts** tab, no OAuth, then point a Download
...
> The same content is available inside the web UI under the **ð Help** tab.

BOT-SYNC also runs on a regular **Raspberry Pi / Linux box / macOS / Windows
PC** via the unified `install/install.py` dispatcher â see
...
| `/etc/firewall.botsync`                    | iptables PREROUTING rule (Host: bot.sync â :8585) |
...
works â `scp`, the bundled `push_files.py`, or copy via USB). On the GL-A1300:
...
That's it. The installer is idempotent â re-run any time.
...
1. **Alias IP on `br-lan`** â derived from the LAN IP by replacing the last
   octet with `.244` (e.g. `192.168.8.1` â `192.168.8.244`). Override with
...
2. **DNS** â dnsmasq is configured with `address=/bot.sync/<alias-ip>` so
...
3. **Port-80 redirect** â a firewall include (`/etc/firewall.botsync`,
...
- **fw4 / nftables.** OpenWrt 22.03+ defaults to nftables. The provided
  ...
  Pass `--no-hostname` to skip the redirect entirely â the daemon still
...
or offline, etc. Configure it on the **ð Notifications** tab â *Add channel*
â kind **Email (SMTP)**. Fields:
...
|---|---|---|---|---|
| Gmail | `smtp.gmail.com` | `465` | SSL | Requires a 16-char *App password* (account â Security â 2-Step â App passwords). |
...
succeeds â see *Auto-sync &amp; outage recovery* below.
jobs, network hangs, and unclean reboots. The mechanisms layered together:

| Layer | What it watches | Kicks in when |
|---|---|---|
| `procd` respawn | Process exit code | Daemon exits for any reason â crash, OOM, unhandled exception. Restart limit is `respawn 60 5 0` (unlimited tries within 60s window, 5s gap). |
| Cron watchdog (`/usr/sbin/botsync-watchdog`) | HTTP responsiveness | 3 consecutive failed pings to `POST /api/watchdog/ping`. Catches the rare âalive but hungâ case where procd alone wouldn't notice. |
...
| Crash-recovery marker | Unclean shutdowns | `<usb>/var/run/botsyncd.running` survives a crash; on next start BOT-SYNC emits `system.crash_recovered` (or `system.watchdog_restart` if cron triggered it) and resets any ârunningâ jobs to error. |
...
- **âï¸ Settings â Reliability &amp; watchdog** in the web UI shows daemon
  PID, uptime, last heartbeat, and whether the cron watchdog has pinged
  recently.
- **Stuck-job timeout (hours)** â same panel. Default `6`. Set to `0` to
...
Wire any of these to your channel of choice (Discord / Slack / Email â¦):

- `system.crash_recovered` â daemon restarted after an unclean shutdown.
- `system.watchdog_restart` â cron watchdog forced a restart.
- `system.shutdown` â clean stop (SIGTERM or service stop).
- `system.error` â fatal unhandled exception (also written to `crash/`).
- `job.interrupted` â a job was running when the daemon went down.
- `job.stuck` â stuck-job timeout cancelled a long-running job.
...
inside the authenticated Dropbox account. So pasting a `dropbox.com/scl/fo/â¦`
or `dropbox.com/sh/â¦` link is not enough on its own.
...
4. In the bot-sync UI, edit the download (or add it again) and set
   **Remote path** to that exact folder name. **Do not leave Remote path
   blank** â leaving it blank tells rclone to copy your *entire* Dropbox
...
5. You only have to do this once. Subsequent syncs and reboots reuse the
   ...
   `directory not found` â at which point you re-do step 1.
...
1. **Create a Google Cloud project** at
   <https://console.cloud.google.com/> â top-bar project picker â
   **New Project** â name it (e.g. *bot-sync*) â **Create**.
2. **Enable the Google Drive API**:
   <https://console.cloud.google.com/apis/library/drive.googleapis.com>
   with that project selected â **Enable**.
3. **Configure the OAuth consent screen**:
   <https://console.cloud.google.com/apis/credentials/consent>
   - User type: **External** â **Create**.
   - App name + support email â **Save and continue**.
   - Scopes: leave default â **Save and continue**.
   - **Test users â Add users**: add the Gmail/Workspace address that
     owns the Drive bot-sync will sync. Without this, sign-in fails
     with *Access blocked: â¦has not completed Google's verification
     process*.
   - **Save and continue** â **Back to dashboard**.
4. **Create OAuth client credentials**:
   <https://console.cloud.google.com/apis/credentials>
   â **Create Credentials â OAuth client ID** â
   Application type **Desktop app** â **Create**.
5. **Copy the Client ID and Client secret** Google shows you.
6. **In bot-sync**: Accounts â **â Connect account** â Provider
   *Google Drive* â name *gdrive* (or anything) â paste both values
   â **ð Sign in with browser**. A new tab opens to Google; sign in,
   approve, return to bot-sync. The remote flips to **OK** when the
   refresh token is written.

When a token later expires/revokes, the UI shows a red **ð
Reauthenticate** button â click it, then **ð Sign in with browser**.
...
It does **not** touch the USB drive â pop the drive into another router that
